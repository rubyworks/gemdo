...
with the `--minor` flag.
  ...
  `cd tmp/example; rock bump --minor`

And we can see that the `minor` number has been incremented.

  package['vers'].assert = '1.1.0'

We can use the `rock bump` command to bump the `major` version by
using the `--major` flag.

  `cd tmp/example; rock bump --major`
